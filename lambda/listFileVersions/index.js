...
function getPrefixFromFilePath(filePath) {
  ...
  // purpose of this is to match older versions of the same stored object name
  return filePath.slice(0, lastSlashIndex + 1) + filePath.slice(lastSlashIndex + 1);
}
...
exports.handler = async (event) => {
  ...
  const listResponse = await s3Client.send(
    new ListObjectVersionsCommand({
      ...
      Prefix: getPrefixFromFilePath(filePath),
    })
  );
